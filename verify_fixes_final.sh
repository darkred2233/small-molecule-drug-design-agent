#!/bin/bash
# 代码结构验证脚本 - 包含 SelfRefutation LLM 质询检查

echo "============================================================"
echo "验证空壳实现修复 - 代码结构检查（含 SelfRefutation）"
echo "============================================================"
echo ""
...
# 测试 4: SelfRefutation LLM 质询 (新增)
echo "测试 4: SelfRefutation LLM 质询"
echo "------------------------------------------------------------"
TOTAL=$((TOTAL + 5))

if grep -q "from medagent.llm import LLMMessage, get_llm_client" src/medagent/services/self_refutation.py; then
    echo "✓ SelfRefutation 已导入 LLM 模块"
    PASS=$((PASS + 1))
else
    echo "✗ SelfRefutation 未导入 LLM 模块"
fi

if grep -q "def _llm_critique" src/medagent/services/self_refutation.py; then
    echo "✓ SelfRefutation 包含 _llm_critique 方法"
    PASS=$((PASS + 1))
else
    echo "✗ SelfRefutation 缺少 _llm_critique 方法"
fi

if grep -q "llm_critique_json" src/medagent/services/self_refutation.py; then
    echo "✓ SelfRefutation 处理 llm_critique_json"
    PASS=$((PASS + 1))
else
    echo "✗ SelfRefutation 未处理 llm_critique_json"
fi

if grep -q "llm_critique_json" src/medagent/db/models.py; then
...
if grep -A 80 "_llm_critique" src/medagent/services/self_refutation.py | grep -q "hidden_risks"; then
    echo "✓ LLM 质询包含隐藏风险检测"
    PASS=$((PASS + 1))
else
    echo "✗ LLM 质询缺少隐藏风险检测"
...
# 测试 5: batch_molecule_task 实现
echo "测试 5: batch_molecule_task 实现"
echo "------------------------------------------------------------"
TOTAL=$((TOTAL + 2))
