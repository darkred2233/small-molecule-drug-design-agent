...
- REINVENT4、AutoGrow4 外部完整系统当前未安装，因此用 RDKit/Datamol 的确定性枚举、标准化、Tanimoto 约束和基础性质评分作为代理适配器，并在响应与标签里显式标记外部适配器待接入。
...
- `strategy_summaries`：每类策略的入库分子 ID、adapter 模式、工具状态、warning、候选来源统计。
...
`input_json` 和 `output_json` 会记录请求参数、seed 数量、工具状态、每类策略统计、warning 和外部 adapter 是否已连接。
...
- 安装并接入真实 REINVENT4 CLI/配置模板，把当前 surrogate adapter 替换为真实任务提交器。
- 安装并接入 AutoGrow4 与 docking 环境，把当前 grow/link surrogate 替换为真实结构导向生成。
