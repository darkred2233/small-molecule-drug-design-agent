...
from medagent.services.rdkit_adapter import validate_smiles_with_rdkit
...
def validate_project_molecules(db: Session, project: Project) -> dict:
    ...
    for molecule in molecules:
        ...
        if result.valid:
            ...
            update_molecule_structure_fields(molecule, result.descriptors or {})
...
def validate_smiles(smiles: str) -> ValidationResult:
    ...
    if rdkit_result.available:
        return ValidationResult(
            valid=rdkit_result.valid,
            labels=rdkit_result.labels,
            reason=rdkit_result.reason,
            descriptors=rdkit_result.descriptors,
        )
    fallback = validate_smiles_lightweight(smiles)
    if fallback.valid:
        fallback.labels = merge_labels(fallback.labels, ["rdkit_unavailable"])
        fallback.descriptors = {
            **(fallback.descriptors or {}),
...
def update_molecule_structure_fields(molecule: Molecule, descriptors: dict) -> None:
    inchi_key = descriptors.get("inchi_key")
    scaffold = descriptors.get("scaffold")
    if inchi_key:
...
def upsert_molecule_property(db: Session, molecule: Molecule, descriptors: dict) -> None:
    ...
    existing.logp = descriptors.get("logp")
    existing.tpsa = descriptors.get("tpsa")
