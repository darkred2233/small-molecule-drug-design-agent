...
import { currentStageProgress, isActiveRound } from '@/lib/round-progress';
...
export function RoundRunPage() {
  ...
  const { data: round } = useQuery<ProjectRound, Error>({ queryKey: ['round', projectId, roundId], queryFn: () => roundsApi.get(projectId!, roundId!), enabled, refetchInterval: (query) => isActiveRound(query.state.data?.status) ? 5000 : false });
  const { data: campaigns = [] } = useQuery<CampaignRun[], Error>({ queryKey: ['campaigns', projectId, roundId], queryFn: () => roundsApi.campaigns(projectId!, roundId!), enabled, refetchInterval: isActiveRound(round?.status) ? 5000 : false });
  const { data: summary } = useQuery<RoundSummary, Error>({ queryKey: ['round-summary', projectId, roundId], queryFn: () => roundsApi.summary(projectId!, roundId!), enabled, refetchInterval: isActiveRound(round?.status) ? 5000 : false });
  const completed = campaigns.filter((campaign) => campaign.status === 'completed').length;
  const progress = currentStageProgress(round?.status, summary?.execution_progress);
  const executionProgress = summary?.execution_progress;
  return <div className="page"><div className="page-heading"><div><p className="eyebrow">第 {round?.round_number || '—'} 轮 · Campaign 执行</p><h1>生成与评估进度</h1><p className="subtle">执行过程由后端编排。此处只显示可验证的状态和结果，不提供没有后端能力支持的暂停或重试操作。</p></div><div className="row"><StatusBadge status={round?.status} /><Link className="button" to={`../ranking`}>查看排名 <ArrowRight size={15} /></Link></div></div>
    ...
    <section className="panel" style={{ marginTop: 18 }}><div className="panel-header"><div className="row"><Activity size={17} color="#176451" /><h2>当前阶段进度</h2></div><span className="score">{progress}%</span></div><div className="panel-body"><div className="progress-bar"><span style={{ width: `${progress}%` }} /></div>{executionProgress ? <div className="stack" style={{ marginTop: 10 }}><strong>{executionProgress.stage}</strong><div className="subtle">{executionProgress.message}</div>{typeof executionProgress.completed_molecules === 'number' && typeof executionProgress.total_molecules === 'number' && <div className="subtle">已处理 {executionProgress.completed_molecules} / {executionProgress.total_molecules} 个分子</div>}</div> : <div className="subtle">{round?.status === 'completed' ? '本轮已完成，排名、报告和下一轮策略建议已归档。' : '正在等待后端安排下一阶段。'}</div>}</div></section>
