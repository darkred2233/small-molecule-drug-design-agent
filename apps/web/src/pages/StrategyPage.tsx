...
function campaignStatus(config: CampaignConfig): 'disabled' | 'ready' {
  return config.enabled === false ? 'disabled' : 'ready';
}
...
function enabledCampaignCount(strategy: StrategyDraft, counts: Record<string, number>): number {
  return Object.entries(strategy.campaign_config).reduce((total, [method, config]) => (
    config.enabled === false ? total : total + (counts[method] ?? campaignCount(config))
  ), 0);
}
