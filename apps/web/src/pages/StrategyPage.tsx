...
function campaignStatus(config: CampaignConfig, count: number): 'disabled' | 'ready' {
  return config.enabled === false || count <= 0 ? 'disabled' : 'ready';
}

function assessmentStatus(isSkipped: boolean | undefined): 'skipped' | 'completed' {
  return isSkipped ? 'skipped' : 'completed';
}

function enabledCampaignCount(strategy: StrategyDraft, counts: Record<string, number>): number {
  return Object.entries(strategy.campaign_config).reduce((total, [method, config]) => (
    config.enabled === false || (counts[method] ?? campaignCount(config)) <= 0
      ? total
      : total + (counts[method] ?? campaignCount(config))
  ), 0);
}

function campaignOverrides(strategy: StrategyDraft | undefined, counts: Record<string, number>, seedIds: string[], externalTopN: number): Record<string, unknown> {
  const campaignConfig = Object.fromEntries(Object.entries(strategy?.campaign_config || {}).map(([method, config]) => {
    ...
    const count = counts[method] ?? campaignCount(config);
    return [method, { ...config, [key]: count, enabled: config.enabled !== false && count > 0 }];
  }));
  const overrides: Record<string, unknown> = {
    campaign_config: campaignConfig,
    assessment_config: { ...(strategy?.assessment_config || {}), top_n: externalTopN },
  };
...
export function StrategyPage() {
  ...
  useEffect(() => {
    ...
    setExternalTopN(strategy.assessment_config?.top_n || 10);
  }, [strategy]);
  ...
  const confirm = useMutation<{ round_id: string; status: string; message: string }, Error>({
    mutationFn: () => roundsApi.confirmStrategy(projectId!, roundId!, { confirmed: true, user_modifications: campaignOverrides(strategy, counts, selectedSeedIds, externalTopN) }),
  ...
  return <div className="page"><div className="page-heading"><div><p className="eyebrow">第 {round?.round_number || '—'} 轮 · 策略审核</p><h1>确认本轮生成与评估策略</h1><p className="subtle">Agent 根据目标、数据完整性、工具状态和上一轮结果拟定方案。任何自然语言或手动修改都会形成可审计的新草案。</p></div><StatusBadge status={round?.status} /></div>
    ...
    {strategy && <div className="section-stack"><section className="panel"><div className="panel-header"><div><h2>策略摘要</h2><p className="subtle">{strategy.objective}</p></div><div className="row"><button className="button" onClick={resetCounts}><RotateCcw size={15} />恢复 Agent 建议</button><button className="button button-primary" onClick={() => { if (window.confirm('确认后将按当前策略执行本轮生成、评估和排名。执行记录会永久归档到本轮。')) confirm.mutate(); }} disabled={!canConfirm || confirm.isPending}><Play size={15} />{confirm.isPending ? '正在执行…' : '确认并执行'}</button></div></div><div className="panel-body section-stack"><div className="notice"><CheckCircle2 size={16} />{strategy.rationale}</div>{selectedSeedIds.length > 0 && <div className="notice"><Edit3 size={16} />本轮已带入你手动选择的 {selectedSeedIds.length} 个上一轮 Seed；确认时将覆盖 Agent 的默认 Seed 选择。</div>}{strategy.warnings.length > 0 && <div className="stack">{strategy.warnings.map((warning, index) => <div key={`${warning}-${index}`} className="notice notice-warning"><AlertTriangle size={16} />{warning}</div>)}</div>}</div></section>
      <section className="panel"><div className="panel-header"><div><h2>Campaign 计划</h2><p className="subtle">在此手动修改数量。更复杂的策略变化可通过右侧 Agent 面板用自然语言提出。</p></div><span className="badge badge-neutral">预计生成 {enabledCampaignCount(strategy, counts)} 个候选</span></div><div className="table-scroll"><table className="data-table"><thead><tr><th>生成方式</th><th>状态</th><th>计划数量</th><th>关键参数</th><th>策略角色</th></tr></thead><tbody>{Object.entries(strategy.campaign_config).map(([method, config]) => { const count = counts[method] ?? campaignCount(config); return <tr key={method}><td><strong>{methodLabel(method)}</strong><div className="mono subtle" style={{ margin: '2px 0 0' }}>{method}</div></td><td><StatusBadge status={campaignStatus(config, count)} /></td><td><input aria-label={`${method} 生成数量`} type="number" min="0" max="1000" disabled={config.enabled === false} value={count} onChange={(event) => setCounts((current) => ({ ...current, [method]: Math.max(0, Number(event.target.value) || 0) }))} style={{ width: 92, height: 32, padding: '0 8px', border: '1px solid #c9d5cf', borderRadius: 4 }} /></td><td>{Object.entries(config).filter(([key, value]) => !['enabled', 'num_molecules', 'sample_count'].includes(key) && value !== null && value !== undefined).slice(0, 3).map(([key, value]) => <div key={key} className="subtle" style={{ margin: '1px 0' }}>{key}: {String(value)}</div>)}</td><td>{method === 'crem' ? '局部结构编辑' : method === 'targetdiff' ? '口袋条件生成' : method === 'autogrow4' ? '口袋引导探索' : '候选生成'}</td></tr>; })}</tbody></table></div></section>
      <div className="two-column"><section className="panel"><div className="panel-header"><h2>Seed 策略</h2></div><div className="panel-body stack"><div><strong>{strategy.seed_policy?.source || '未指定'}</strong><p className="subtle">{strategy.seed_policy?.description || '由 Agent 根据当前数据选择。'}</p></div><div className="row-wrap">{strategy.seed_policy?.top_n && <span className="badge badge-neutral">Top {strategy.seed_policy.top_n}</span>}{strategy.seed_policy?.molecule_ids?.map((id) => <span className="badge badge-neutral mono" key={id}>{id}</span>)}</div></div></section><section className="panel"><div className="panel-header"><h2>评估边界</h2></div><div className="panel-body stack"><div className="row" style={{ justifyContent: 'space-between' }}><span>对接评估</span><StatusBadge status={assessmentStatus(strategy.assessment_config?.skip_docking)} /></div><div className="row" style={{ justifyContent: 'space-between' }}><span>ADMET 评估</span><StatusBadge status={assessmentStatus(strategy.assessment_config?.skip_admet)} /></div><div className="row" style={{ justifyContent: 'space-between' }}><span>合成可行性</span><StatusBadge status={assessmentStatus(strategy.assessment_config?.skip_synthesis)} /></div><div className="field"><label htmlFor="external-top-n">外部精筛 Top N</label><input id="external-top-n" type="number" min="1" max="200" value={externalTopN} onChange={(event) => setExternalTopN(Math.max(1, Math.min(200, Number(event.target.value) || 1)))} /></div></div></section></div>
